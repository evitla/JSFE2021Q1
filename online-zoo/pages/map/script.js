...
const markers = document.querySelectorAll(".map-marker_pointer");

const mapCarousel = new Carousel(
  "map__carousel",
  markers,
  numOfSlidesToMove=(window.outerWidth < 1626) ? 1 : 0
);

window.addEventListener("resize", () => mapCarousel.numOfSlidesToMove = (window.outerWidth < 1626) ? 1 : 0);

mapCarousel.element.addEventListener("input", mapCarousel.moveSlidesByInput);
mapCarousel.element.addEventListener("click", mapCarousel.moveSlidesByClickArrowButtons);
mapCarousel.element.addEventListener("click", mapCarousel.moveSlidesByClick);
markers.forEach(marker => marker.addEventListener("click", mapCarousel.moveSlidesByClickMarker));
